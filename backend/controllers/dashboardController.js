...
const getZoneDashboard = async (req, res, next) => {
  try {
    ...
    const centres = await query(
      `SELECT c.id, c.centre_code, c.centre_name, c.city, c.total_capacity,
              ...
              (SELECT COUNT(*) FROM users u3
               JOIN staff_attendance sa ON sa.user_id = u3.id AND sa.exam_date = $2 AND sa.exam_type = $3
              ...
              (SELECT COUNT(*) FROM issues i WHERE i.centre_id = c.id AND i.exam_date = $2 AND i.exam_type = $3 AND i.status = 'open') AS open_issues,
              (SELECT COUNT(*) FROM servers s2 WHERE s2.centre_id = c.id AND s2.is_active = TRUE) AS server_count,
              -- Check if this centre has a schedule for this date and exam_type
              EXISTS (
    ...
    // Filter only centres that have schedule for this exam_type
    const filteredCentres = centres.rows.filter(c => c.has_schedule);

    const total    = filteredCentres.length;
    const allDone  = filteredCentres.filter(c => parseInt(c.staff_reported) === parseInt(c.total_staff) && c.checklist_submitted).length;
    const partial  = filteredCentres.filter(c => parseInt(c.staff_reported) > 0 && (parseInt(c.staff_reported) < parseInt(c.total_staff) || !c.checklist_submitted)).length;
    const notStart = filteredCentres.filter(c => parseInt(c.staff_reported) === 0).length;
    ...
    const totals = await query(
      `SELECT
        ...
        (SELECT COUNT(DISTINCT u.centre_id)
         FROM users u
         JOIN staff_attendance sa ON sa.user_id = u.id AND sa.exam_date = $1 AND sa.exam_type = $2
         WHERE u.centre_id IS NOT NULL) AS reporting_centres,
        (SELECT COALESCE(SUM(present),0) FROM batch_attendance WHERE exam_date = $1) AS total_present,
        (SELECT COUNT(*) FROM issues WHERE exam_date = $1 AND exam_type = $2 AND status = 'open') AS open_issues`,
      [date, exam_type]
    );

    const stats = totals.rows[0];
    stats.not_started = parseInt(stats.scheduled_centres) - parseInt(stats.reporting_centres);
    if (stats.not_started < 0) stats.not_started = 0;

    // Zone wise summary — only count centres scheduled for this exam_type
    const zones = await query(
      `SELECT z.id, z.zone_code, z.zone_name, z.state,
              COUNT(DISTINCT c.id) AS total_centres,
              COUNT(DISTINCT CASE WHEN ss.exam_type = $2 AND ss.exam_date = $1 THEN sv.centre_id END) AS scheduled_centres,
       ...
       LEFT JOIN session_schedule ss ON ss.server_id = sv.id
