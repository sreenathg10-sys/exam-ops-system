    ...
    if (existing.rows[0]) {
      await query(
        `UPDATE staff_attendance
         SET reported_at = NOW(), reported_geo_lat = $1, reported_geo_lng = $2, updated_at = NOW()
...
// POST upload selfie — uses buffer from memoryStorage
const uploadSelfie = async (req, res, next) => {
  try {
    ...
    const storagePath = `selfies/${req.user.id}_${today}_selfie${ext}`

    // Upload buffer directly — no disk write needed
    ...
    await query(
      `INSERT INTO staff_attendance (user_id, exam_date, selfie_url)
       VALUES ($1, $2, $3)
       ON CONFLICT (user_id, exam_date)
       DO UPDATE SET selfie_url = $3, updated_at = NOW()`,
...
const saveGeoTag = async (req, res, next) => {
  try {
    ...
    if (!geo_lat || !geo_lng) {
      return res.status(400).json({ success: false, message: 'geo_lat and geo_lng are required' })
    }
    const today = new Date().toISOString().split('T')[0]
    ...
    await query(
      `INSERT INTO staff_attendance (user_id, exam_date, geo_lat, geo_lng, geo_captured_at)
       VALUES ($1, $2, $3, $4, NOW())
       ON CONFLICT (user_id, exam_date)
       DO UPDATE SET geo_lat = $3, geo_lng = $4, geo_captured_at = NOW(), updated_at = NOW()`,
...
// GET centre attendance (for dashboard)
const getCentreAttendance = async (req, res, next) => {
  try {
    const { centre_code } = req.params
    const examDate = req.query.date || new Date().toISOString().split('T')[0]
