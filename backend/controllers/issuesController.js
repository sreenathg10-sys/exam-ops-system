  ...
  try {
    ...
    const result = await query(
      `INSERT INTO issues (reported_by, centre_id, server_id, exam_date, exam_type, section_code, issue_type, severity, app_roll_no, description)
       ...
       RETURNING id`,
      [req.user.id, req.user.centre_id, req.user.server_id, today, exam_type || 'live', section_code || null, issue_type, severity, app_roll_no || null, description]
    );
    ...
    if (req.files && req.files.length > 0) {
      for (const file of req.files) {
        const storagePath = `issues/${issue_id}_${Date.now()}${path.extname(file.originalname)}`;
...
const updateIssueStatus = async (req, res, next) => {
  try {
    ...
    await query(
      `UPDATE issues SET status=$1, resolved_at = CASE WHEN $1='resolved' THEN NOW() ELSE NULL END, updated_at=NOW() WHERE id=$2`,
      [status, id]
    );
