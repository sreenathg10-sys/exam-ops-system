...
export default function MasterDashboard() {
  ...
  useEffect(() => { if (activeTab === 'issues') loadIssues() }, [activeTab, date, examType])
  ...
  const loadIssues = async () => {
    try {
      const res = await issuesAPI.all({ date, status: 'open', exam_type: examType })
  ...
  const downloadReport = async (type) => {
    try {
      ...
      downloadBlob(res.data, `${type}_report_${date}.xlsx`)
              ...
              {['mock','live'].map(t => (
                <button key={t} onClick={() => setExamType(t)}
                  className={`px-4 py-2 text-xs font-bold capitalize transition-all ${
                    examType === t
                      ? t === 'live' ? 'bg-red-600 text-white' : 'bg-blue-700 text-white'
        ...
        {data?.stats && (
          <div className="grid grid-cols-2 sm:grid-cols-5 gap-3 mb-6">
            {[
              ...
              { label: 'Scheduled',         val: data.stats.scheduled_centres, bg: 'bg-blue-50',   text: 'text-blue-700'   },
              { label: 'Reporting',         val: data.stats.reporting_centres, bg: 'bg-green-50',  text: 'text-green-700'  },
              { label: 'Candidates Present',val: data.stats.total_present,     bg: 'bg-indigo-50', text: 'text-indigo-700' },
              ...
              <div key={label} className={`${bg} rounded-xl p-4 text-center border border-gray-100`}>
                <div className={`text-2xl sm:text-3xl font-black ${text}`}>{val ?? '0'}</div>
          ...
          loading
            ...
            : (
              <div className="space-y-3">
                {data?.zones?.filter(z => parseInt(z.scheduled_centres) > 0).length === 0 && (
                  <EmptyState icon={Map} title="No data" desc={`No ${examType} exam scheduled on ${fmtDate(date)}`} />
                )}
                {data?.zones?.filter(z => parseInt(z.scheduled_centres) > 0).map(z => {
                  const scheduled = parseInt(z.scheduled_centres) || 0
                  const reporting = parseInt(z.reporting_centres)  || 0
                  const pct       = scheduled > 0 ? Math.round((reporting / scheduled) * 100) : 0
                  return (
                    <div key={z.zone_code}
                      onClick={() => setSelectedZone(z)}
                      className={`card p-4 border-l-4 cursor-pointer hover:shadow-md transition-all ${
                        pct === 100 ? 'border-l-green-500' : pct > 0 ? 'border-l-amber-400' : 'border-l-red-500'
                      }`}>
                      <div className="flex items-start justify-between mb-3 gap-2">
                        <div>
                          <div className="font-semibold text-gray-900">{z.zone_name}</div>
                          <div className="text-xs text-gray-400">{z.state} · {z.scheduled_centres} centres · {examType}</div>
                        </div>
                      ...
                      <div className="grid grid-cols-5 gap-2 text-center">
                        <div className="bg-gray-50 rounded-lg p-2">
                          <div className="text-sm font-bold text-gray-700">{reporting}/{scheduled}</div>
                          <div className="text-xs text-gray-400">Reporting</div>
                        </div>
                        {['b1_present','b2_present','b3_present'].map((k, i) => (
                          <div key={k} className="bg-blue-50 rounded-lg p-2">
                            <div className="text-sm font-bold text-blue-700">{z[k] || '0'}</div>
                            <div className="text-xs text-gray-400">B{i+1}</div>
                          </div>
                        ))}
                        <div className={`rounded-lg p-2 ${parseInt(z.open_issues) > 0 ? 'bg-red-50' : 'bg-gray-50'}`}>
                          <div className={`text-sm font-bold ${parseInt(z.open_issues) > 0 ? 'text-red-600' : 'text-gray-500'}`}>
                            {z.open_issues || '0'}
                          </div>
        ...
        {activeTab === 'issues' && (
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              <div className="text-sm text-gray-500">{issues.length} open · {fmtDate(date)} · {examType}</div>
              <button onClick={loadIssues} className="p-1.5 rounded-lg hover:bg-gray-100 text-gray-400">
                <RefreshCw className="h-4 w-4" />
              </button>
            </div>
            {issues.length === 0
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="font-semibold text-gray-800 capitalize text-sm">
                        {issue.issue_type?.replace('_',' ')} — {issue.centre_name}
                      </div>
                      <div className="text-xs text-gray-500 mt-0.5">
                        {issue.zone_name} · {issue.city}{issue.section_code ? ` · ${issue.section_code}` : ''}
                      </div>
        ...
        {activeTab === 'reports' && (
          <div className="space-y-4">
            <div className="text-sm text-gray-500 mb-2">
              Reports for <strong>{fmtDate(date)}</strong> · <span className="capitalize font-semibold">{examType}</span>
            </div>
            {[
              { type: 'zone',   label: 'Zone Wise Attendance Summary',  desc: 'All zones · Staff + candidate attendance' },
              { type: 'centre', label: 'Centre Wise Detailed Report',   desc: 'All centres · Checklist + batch + issues' },
            ].map(({ type, label, desc }) => (
              <div key={type} className="card p-4 flex items-center justify-between gap-4">
                <div>
                  <div className="font-semibold text-gray-800 text-sm">{label}</div>
                  <div className="text-xs text-gray-400 mt-0.5">{desc} · Excel</div>
                </div>
                <button onClick={() => downloadReport(type)} className="btn-success flex items-center gap-2 text-sm flex-shrink-0">
    ...
    <div className="space-y-5">
      ...
      <div className="card p-4 space-y-3">
        ...
        <button onClick={handleUpload} disabled={!file || uploading}
          ...
          {uploading ? <><Spinner size="sm" /> Importing...</> : 'Upload & Import'}
        </button>

        {/* Result */}
        {result && (
          <div className={`rounded-xl p-4 ${result.success ? 'bg-green-50 border border-green-200' : 'bg-red-50 border border-red-200'}`}>
            <div className={`font-bold text-sm mb-3 ${result.success ? 'text-green-800' : 'text-red-800'}`}>
              {result.success ? 'Import completed successfully' : 'Validation failed — fix errors and re-upload'}
            </div>
                ...
                {Object.entries(result.data.summary).map(([k, v]) => (
                  <div key={k} className="bg-white rounded-lg p-2 text-center border border-green-100">
                    <div className="font-bold text-gray-800 text-sm">
                      {typeof v === 'object'
                        ? `+${v.inserted || 0} new / ${v.updated || 0} updated${v.skipped ? ` / ${v.skipped} skipped` : ''}`
                        : v}
                    </div>
                    <div className="text-gray-500 capitalize mt-0.5">{k.replace(/_/g, ' ')}</div>
                  </div>
                ))}
              </div>
            )}
              ...
              <div className="space-y-3 max-h-96 overflow-y-auto">
                {result.data?.errors && Object.keys(result.data.errors).length > 0
                  ? Object.entries(result.data.errors).map(([sheet, errs]) => (
                    <div key={sheet} className="bg-white rounded-lg border border-red-200 overflow-hidden">
                      <div className="bg-red-100 px-3 py-2 flex items-center justify-between">
                        <span className="text-xs font-bold text-red-800">Sheet: {sheet}</span>
                        <span className="text-xs text-red-600">{Array.isArray(errs) ? errs.length : 0} error(s)</span>
                      </div>
                      <div className="p-3 space-y-1">
                        {Array.isArray(errs) && errs.map((e, i) => (
                  ...
                  : (
                    <div className="bg-white rounded-lg p-3 border border-red-200">
                      <div className="text-xs font-bold text-red-700 mb-1">Server error:</div>
